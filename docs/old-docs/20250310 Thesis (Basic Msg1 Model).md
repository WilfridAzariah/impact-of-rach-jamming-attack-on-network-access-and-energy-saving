...
### 1.1. PRACH Msg1 Attack Overview
...
xychart-beta
    ...
    x-axis RO 11 --> 20
    y-axis "Power (dB)" 15 --> 60
    bar [43.4, 43.4, 43.4, 43.4, 43.4, 43.4, 43.4, 43.4, 43.4, 43.4]
    line [17.78, 22.3, 26.28, 29.8, 32.86, 35.56, 37.92, 39.94, 41.74, 43.36]
    line [29.78, 34.3, 38.28, 41.8, 44.86, 47.56, 49.92, 51.94, 53.74, 55.36]
```
...
```math
p_{th,i} = (1 - \alpha) * p_{th,i-1} + \alpha * p_{measured,i-1}
```
3. $p_{measured,i}$:
```math
p_{measured,i} =
\begin{cases} 
p_{attacker}, & \text{if } i > 0 \text{ and } (i-1) \bmod T_a = 0 \\ 
p_{noise}, & \text{otherwise} 
\end{cases}
```
4. Noise threshold for when attacker start early for $j$ RAO is equal to evaluating $P_{noise,i+1}$ over the range $i \in [1, j+1]$ and taking the $P_{noise,j+1}$
5. UE's Msg1 Success with Noise threshold at the $i$ RAO is:
```math
P_{S,i} =
\begin{cases} 
1, & \text{if } p_{UE} > (p_{th,i} + \delta) \\ 
0, & \text{otherwise} 
\end{cases}
```
