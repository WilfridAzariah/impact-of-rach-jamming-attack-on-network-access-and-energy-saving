# Thesis (Test of PRACH Attacker for Basic Msg3 Model)

###### tags: `2025`


**Goal:**
- [x] [Test PRACH Attacker for Basic Msg3 Model]()
...
**Table of Contents:**
- [Thesis (Test of PRACH Attacker for Basic Msg3 Model)](#thesis--test-of-prach-attacker-for-basic-msg3-model-)
          + [tags: `2025`](#tags---2025-)
  * [0. Summary](#0-summary)
...
| Attacker receive Msg2 from gNB | :heavy_check_mark: |
| [Attacker Send Msg3 to gNB]() | :heavy_check_mark: |
...
| OS         | Ubuntu 22.04.4 LTS (jammy) |
| Kernel     | 6.8.0-52-generic           |
| OAI Commit | 8117e46aa2e08f5d0d0859157612c8a070ad6a72 (HEAD -> develop, origin/develop, origin/HEAD)                           |
...
<b>7. Clone MSG3 Attacker and checkout to rework_UE branch</b>

```shell=
git clone https://github.com/Richard-yq/OAI-UE-MSG3-attacker.git
cd  OAI-UE-MSG3-attacker
git checkout develop
```
...
<b>11. This is a figure to describe what is actually done by Richard to convert OAI UE into a Msg3 Attacker</b>
```mermaid
flowchart TD
    A(("nr_ue_prach_procedures()")) --> B["..."]
    B --> C["//generate_nr_prach()"]
    C --> D["..."]
```

```mermaid
flowchart TD
    A(("nr_ue_process_rar()")) --> B["..."]
    B --> C{"//rarh->RAPID ==
    //preamble_index"}
    C --> |yes| D["handle_rar_reception()"]
    D --> E["..."]
    E --> F["..."]
    C --> |no| F
```
...
```mermaid
flowchart TD
    A(("nr_ue_ul_scheduler()")) --> B["..."]
    B --> C["nr_get_Msg3_MsgA
    _PUSCH_payload()"]
    C --> D["..."]
    D --> E["//nr_Msg3_transmitted()"]
    E --> F["..."]
```

```mermaid
flowchart TD
    A(("nr_ulsch_encoding()")) --> B["..."]
...
- We can see from gNB log that it gets UE's Msg1 with preamble ID 20

![image](https://github.com/user-attachments/assets/eb8e16ac-ee84-41c6-a27b-9ee745b27b23)

- We can see from Attacker's Log, it gets gNB's Msg2 for preamble ID 20 and is transmitting Msg3

![image](https://github.com/user-attachments/assets/26089a80-68d2-469f-8790-4f389f018319)

- In gNB log, it shows that the Msg3 payload received by the gNB is wrong

![image](https://github.com/user-attachments/assets/abbf09ac-9bfa-432b-89a8-a5293895e36d)
