  ...
  * [1. PRACH Attacker for Basic Msg1 Model](#1-prach-attacker-for-basic-msg1-model)
    ...
    + [1.4. Run](#14-run)
      ...
      - [1.4.2. Result](#142-result)
        ...
        * [1.4.2.5. Modify gNB Alpha Value](#1425-modify-gnb-alpha-value)
        * [1.4.2.6. Modify gNB Delta Value](#1426-modify-gnb-delta-value)
        * [1.4.2.7. Modify Attacker Msg1 Energy](#1427-modify-attacker-msg1-energy)
    + [1.5. Results Compilation and Visualization](#15-results-compilation-and-visualization)
      - [1.5.1. Parameters Description](#151-parameters-description)
      - [1.5.2. Results](#152-results)
  * [2. PRACH Attacker for Basic Msg1 Model with UE](#2-prach-attacker-for-basic-msg1-model-with-ue)
    + [2.0. Minimum Requirement](#20-minimum-requirement)
      - [2.0.1. OAI gNB with USRP](#201-oai-gnb-with-usrp)
    ...
    + [2.2. Environment](#22-environment)
      - [2.2.1. OAI gNB](#221-oai-gnb)
      - [2.2.2. OAI UE for attacker](#222-oai-ue-for-attacker)
    + [2.3. Compile](#23-compile)
      - [2.3.1. Compile gNB](#231-compile-gnb)
      - [2.3.2. Compile Attacker](#232-compile-attacker)
    + [2.4. Run](#24-run)
      - [2.4.1. Configuration](#241-configuration)
        * [2.4.1.1. gNB Configuration](#2411-gnb-configuration)
        * [2.4.1.2. Attacker Configuration](#2412-attacker-configuration)
      - [2.4.2. Result](#242-result)
        * [2.4.2.1. Initial Run](#2421-initial-run)
        * [2.4.2.2. Modify Attacker Msg1 Energy](#2422-modify-attacker-msg1-energy)
        * [2.4.2.3. Modify gNB to reject preamble with specific id](#2423-modify-gnb-to-reject-preamble-with-specific-id)
        * [2.4.2.4. Modify gNB to sense the first time UE send preamble](#2424-modify-gnb-to-sense-the-first-time-ue-send-preamble)
        * [2.4.2.5. Modify gNB to reject first k UE preambles](#2425-modify-gnb-to-reject-first-k-ue-preambles)
    + [2.5. Results Compilation and Visualization](#25-results-compilation-and-visualization)
      - [2.5.1. Parameters Description](#251-parameters-description)
      - [2.5.2. Results](#252-results)
...
<b>11. This is a figure to describe what is actually done by Richard to convert OAI UE into a Msg1 Attacker</b>

```mermaid
flowchart TD
    A(("nr_ue_ul_scheduler()")) --> B{"ra_state ==
    nrRA_GENERATE_PREAMBLE"}
    B --> |yes| C["nr_ue_prach_scheduler()"]
    C --> D["..."]
    B --> |no| D
```
...
flowchart TD
    ...
    C --> D["//ra_state =
    //nrRA_WAIT_RAR"]
    D --> E["//nr_timer_start(response_window_timer)"]
    E --> F((return))
    B --> |no| F
```
...
<b>0.1. Here is an illustration of modifying attacker period to every 2 frames</b>
...
<b>0.2. Hardcode prach_ConfigurationIndex to 149 in `openair2/LAYER2/NR_MAC_UE/config_ue.c` in 2 functions</b>
...
<b>0.3. Recompile Attacker following [1.3.2.](#132-compile-attacker)</b>
![image](https://github.com/user-attachments/assets/c8f094ed-7492-47c5-b204-e02d2bfa7b16)
...
## 2. PRACH Attacker for Basic Msg1 Model with UE
...
<b>0.1. Here is an illustration of sensing the first UE preamble</b>
...
<b>0.2. Add flag for the first preamble not equal to 1 in `openair1/SCHED_NR/nr_prach_procedures.c`</b>
![image](https://github.com/user-attachments/assets/6f066e73-93af-45b4-91b9-6eecc8698aaa)

<b>0.3. Recompile gNB following [2.3.1.](#231-compile-gnb)</b>

##### 2.4.2.5. Modify gNB to reject first k UE preambles

<b>0.1. Here is an illustration of rejecting first k UE preambles</b>
...
gantt
    ...
    RO 6 : active, rao5, 50, 5m
    section Attacker
    ‎PID=1 : crit, att1, 10, 5m
    ‎PID=1 : crit, att3, 30, 5m
    ‎PID=1 : crit, att5, 50, 5m
    j=1 : done, j1, 10, 10m
    section UE
    ‎PID=9 : ue1, 00, 5m
    ‎PID=8 : ue1, 20, 5m
...
<b>0.2. Add counter for the preamble not equal to 1 in `openair1/SCHED_NR/nr_prach_procedures.c`</b>
![image](https://github.com/user-attachments/assets/171bcad7-b3db-4b72-a022-3ad0fb524afc)

<b>0.3. Recompile gNB following [2.3.1.](#231-compile-gnb)</b>

##### 2.4.2.6. Modify gNB to broadcast different prach_configurationIndex
...
##### 2.4.2.7. Modify attacker to use specific Preamble ID
