  ...
  if (prach_id >= 0) {
	...
	gNB->prach_vars.rxsigF[aa] = ru->prach_rxsigF[prach_oc][ru_aa];
      ...
      if ((gNB->prach_energy_counter == 100) && (max_preamble_energy[0] > gNB->measurements.prach_I0 + gNB->prach_thres)
		...
		if (max_preamble[0] == wilfrid_wus_index) { ///// WILFRID - Check if UE received WUS /////
          printf("%d.%d Received WUS preamble %d\n", frame, prach_start_slot, max_preamble[0]);
          if (wilfrid_gnb_mode == 1) { ///// WILFRID - Check if gNB is in sleep mode when UE received WUS /////
            wilfrid_wus_received = 1; ///// WILFRID - Flag that gNNB receive WUS /////
          }
        }
        else { ///// WILFRID - Normal condition /////
		///// WILFRID - ADD THIS BLOCK OF CODE - WILFRID /////
        LOG_A(NR_PHY,
      ...
      gNB->measurements.prach_I0 = ((gNB->measurements.prach_I0*wilfrid_gamma)>>10) + ((max_preamble_energy[0]*wilfrid_beta)>>10); ///// WILFRID - Msg1 beta and gamma /////
