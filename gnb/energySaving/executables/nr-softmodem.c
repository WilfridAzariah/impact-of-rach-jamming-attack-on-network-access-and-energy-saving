...
int start_L1L2(module_id_t gnb_id)
{
  ...
  RU_t *ru = RC.ru[0];
  printf("wilfrid_gnb_mode = %d\n", wilfrid_gnb_mode);
  if (wilfrid_gnb_mode == 1) { ///// WILFRID - gNB in sleep, hardcode to SISO /////
    ru->nb_tx = 1;
    ru->nb_rx = 1;
  } else if (wilfrid_gnb_mode == 2) { ///// WILFRID - gNB in active, hardcode to MIMO /////
    ru->nb_tx = 2;
    ru->nb_rx = 2;
  }
  ///// WILFRID - ADD THIS BLOCK OF CODE - WILFRID /////
...
///// WILFRID - ADD THIS BLOCK OF CODE - WILFRID /////
static int start_modem() ///// WILFRID - Copy of start_modem() function /////
{
  start_L1L2(0);
  printf("OK\n");
  return 0;
...
static int stop_modem() ///// WILFRID - Copy of stop_modem() function /////
{
  stop_L1(0);
  printf("OK\n");
  return 0;
...
void *gNB_state_manager_thread(void *arg) { ///// WILFRID - gNB's state manager thread /////
  ///// WILFRID - Always start in sleep /////
  printf("=========gNB operating in sleep==========\n");
  while (wilfrid_use_sleep_mode) {
    if (wilfrid_gnb_mode == 1 && wilfrid_wus_received) {
      wilfrid_wus_received = 0; ///// WILFRID - reset WUS received flag /////
      stop_modem();
      wilfrid_gnb_mode = 2; ///// WILFRID - Change gNB to active /////
      start_modem();

      printf("==========gNB operating in active==========\n");
      time_t start = time(NULL);
      while (difftime(time(NULL), start) < wilfrid_active_standby_time) { ///// WILFRID - gNB stay active for T_(active) /////
        ///// WILFRID - Future work should include an UL Traffic Detector function here /////
        sleep(1);
      }
      stop_modem();
      wilfrid_gnb_mode = 1; ///// WILFRID - Change gNB to sleep /////
      start_modem();
...
//// Init the E2 Agent
  ...
  pthread_create(&state_manager_thread, NULL, gNB_state_manager_thread, NULL); ///// WILFRID - Create gNB_state_manager_thread /////
  ///// WILFRID - ADD THIS BLOCK OF CODE - WILFRID /////
